...
# =========================
stage3_root = Path(f"{config.args.cls_dir}/stage_3")/method / current_map/config.args.run_name
model_root  = Path(config.args.model_dir) / method / str(current_map)

# 你的資料夾結構：fixed_train_data_.../train  &  /val
train_dir = stage3_root / "train"
val_dir   = stage3_root / "val"
...
output_dir = model_root / config.args.run_name
...
run_info = {
    ...
    "data_fixed_dir": str(stage3_root),
    "train_dir": str(train_dir),
    "val_dir": str(val_dir),
    "cfg_base": str(cfg),
}
...
actual_train_loss_used = used_tl
print(f"✅ candidates after train_loss filter: {len(cands_tl)} (train_loss_max_used={actual_train_loss_used})")
...
def pick_closest_to_target_under_cap(cands):
    ...
    for r in cands:
        ...
        if best_key is None or key < best_key:
            ...
            best = r
    return best
def pick_best_epoch_from_raw_peak_before_limit(by_epoch, epoch_limit):
    peak_rows = []
    for e in range(1, epoch_limit + 1):
        if e in by_epoch:
            rr = dict(by_epoch[e])
    ...
    best_f1 = max(r["val_f1"] for r in peak_rows)
    ties = [r for r in peak_rows if abs(r["val_f1"] - best_f1) <= LOW_F1_EPS]
    ties = sorted(ties, key=lambda r: r["epoch"])
...
if best is None:
    ...
    actual_train_loss_used = used_tl2
if best is None:
...
low_f1_rescue_triggered = False
original_best_epoch = best_epoch
original_reason = reason

if r_best["val_f1"] < LOW_F1_RESCUE_THRESHOLD:
    rescue_pick = pick_best_epoch_from_raw_peak_before_limit(by_epoch, epoch_limit)
    if rescue_pick is not None and rescue_pick["val_f1"] > r_best["val_f1"] + LOW_F1_EPS:
        low_f1_rescue_triggered = True
        best_epoch = int(rescue_pick["epoch"])
        r_best = by_epoch[best_epoch]
        reason = (
            f"{original_reason} -> low_f1_rescue triggered "
            f"(original val_f1={by_epoch[original_best_epoch]['val_f1']:.4f} < {LOW_F1_RESCUE_THRESHOLD}), "
            f"rescued by raw peak search in epochs<= {epoch_limit}"
        )
...
select_info = {
    ...
    "train_loss_max_used": actual_train_loss_used,
    ...
    "low_f1_rescue_threshold": LOW_F1_RESCUE_THRESHOLD,
    "low_f1_rescue_triggered": low_f1_rescue_triggered,
    "original_selected_epoch_before_low_f1_rescue": original_best_epoch,
    "original_reason_before_low_f1_rescue": original_reason,
}
